...
let new_proposal 
    ...
    let {voting_context; events} = Voting.get_voting_context storage in
    ...
    events, { storage with voting_context = { voting_context with proposals = updated_proposals } }
...
let vote
    ...
    events, { storage with voting_context = { voting_context with promotion = Some updated_promotion } }
...
let get_last_winner_payload
        ...
        : pt =
    let _ = Utils.assert_no_xtz_in_transaction () in
    let {voting_context; events=_} = Voting.get_voting_context storage in
