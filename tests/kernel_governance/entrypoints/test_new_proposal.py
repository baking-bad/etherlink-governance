...
from tests.helpers.errors import (
    ...
    PROPOSAL_ALREADY_CREATED, PROPOSER_NOT_ALLOWED, UPVOTING_LIMIT_EXCEEDED, SENDER_NOT_KEY_HASH_OWNER, 
...
class KernelGovernanceNewProposalTestCase(BaseTestCase):
    def test_should_fail_if_preimage_hash_has_incorrect_size(self) -> None:
        ...
        with self.raisesMichelsonError(INCORRECT_PREIMAGE_HASH_SIZE):
            governance.using(baker).new_proposal(pkh(baker), bytes.fromhex('009279df4982e47cf101e2525b605fa06cd3ccc0f67d1c792a6a3ea56af9606a')).send()
        with self.raisesMichelsonError(INCORRECT_PREIMAGE_HASH_SIZE):
            governance.using(baker).new_proposal(pkh(baker), bytes.fromhex('009279df4982e47cf101e2525b605fa06cd3ccc0f67d1c792a6a3ea56af9606abcde')).send()
        governance.using(baker).new_proposal(pkh(baker), bytes.fromhex('009279df4982e47cf101e2525b605fa06cd3ccc0f67d1c792a6a3ea56af9606abc')).send()
    ...
    def test_should_fail_if_current_period_is_not_proposal(self) -> None:
        ...
        # Period index: 0. Block: 2 of 2
        governance.using(baker).new_proposal(pkh(baker), '010101010101010101010101010101010101010101010101010101010101010101').send()
        self.bake_block()
    ...
    def test_should_fail_if_new_proposal_limit_is_exceeded(self) -> None:
        ...
        governance.using(baker).new_proposal(pkh(baker), '020202020202020202020202020202020202020202020202020202020202020202').send()
        self.bake_block()

        with self.raisesMichelsonError(UPVOTING_LIMIT_EXCEEDED):
            governance.using(baker).new_proposal(pkh(baker), '030303030303030303030303030303030303030303030303030303030303030303').send()
    ...
    def test_should_create_new_proposal_with_correct_parameters(self) -> None:
        ...
        kernel_hash1 = '010101010101010101010101010101010101010101010101010101010101010101'
        # Period index: 0. Block: 1 of 5
        ...
        kernel_hash2 = '020202020202020202020202020202020202020202020202020202020202020202'
        # Period index: 0. Block: 2 of 5
