...
class ProposersGovernanceCheckAddressInLastWinner(BaseTestCase):
    def test_should_fail_if_no_winner_yet(self) -> None:
        ...
        with self.raisesMichelsonError(LAST_WINNER_NOT_FOUND):
            governance.using(baker).check_proposer_in_last_winner(TEST_ADDRESSES_SET[0])
    ...
    def test_should_check_correctly_for_originated_contract_with_winner(self) -> None:
        ...
        assert governance.using(sender).check_proposer_in_last_winner(pkh(allowed_baker)) == True
        assert governance.using(sender).check_proposer_in_last_winner(pkh(another_allowed_baker)) == True
        assert governance.using(sender).check_proposer_in_last_winner(pkh(sender)) == False
    ...
    def test_should_check_correctly_after_full_voting_cycle_with_new_winner(self) -> None:
        ...
        assert governance.using(baker).check_proposer_in_last_winner(pkh(previously_allowed_baker)) == True
        assert governance.using(baker).check_proposer_in_last_winner(pkh(new_allowed_baker)) == False
        assert governance.using(baker).check_proposer_in_last_winner(pkh(new_another_allowed_baker)) == False
        ...
        assert governance.using(baker).check_proposer_in_last_winner(pkh(previously_allowed_baker)) == False
        assert governance.using(baker).check_proposer_in_last_winner(pkh(new_allowed_baker)) == True
        assert governance.using(baker).check_proposer_in_last_winner(pkh(new_another_allowed_baker)) == True
